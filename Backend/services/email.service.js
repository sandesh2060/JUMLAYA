...
// 🚫 EMAIL DISABLED TEMPORARILY - all methods stubbed
// ============================================
...
class EmailService {
  constructor() {
    // 🚫 DISABLED: transporter not initialized
    // this.transporter = nodemailer.createTransporter({ ... });
  }

  async sendEmail(options) {
    // 🚫 EMAIL DISABLED
    console.log(`📧 [EMAIL DISABLED] Would send to: ${options.to} | Subject: ${options.subject}`);
    return true;
  }

  async sendOrderConfirmation(order) {
    console.log(`📧 [EMAIL DISABLED] Order confirmation for order: ${order?.orderId || order?._id}`);
    return true;
  }

  async sendOrderCancellation(order) {
    console.log(`📧 [EMAIL DISABLED] Order cancellation for order: ${order?.orderId || order?._id}`);
    return true;
  }

  async sendOrderStatusUpdate(order, newStatus) {
    console.log(`📧 [EMAIL DISABLED] Order status update: ${order?.orderId || order?._id} → ${newStatus}`);
    return true;
  }

  async sendAbandonedCartEmail(cart) {
    console.log(`📧 [EMAIL DISABLED] Abandoned cart email for cart: ${cart?._id}`);
    return true;
  }
}

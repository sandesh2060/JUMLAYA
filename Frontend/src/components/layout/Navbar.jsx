    ...
    <>
      ...
      {notification && (
        <div className="fixed top-4 right-4 z-[100] animate-slide-in-right max-w-[calc(100vw-2rem)]">
          <div className={`px-4 py-3 sm:px-6 sm:py-4 rounded-xl shadow-2xl border-l-4 flex items-center gap-2 sm:gap-3 min-w-[280px] backdrop-blur-sm ${
            ...
            <div className="flex-shrink-0">
              {notification.type === "success" ? <CheckCircle className="w-5 h-5 sm:w-6 sm:h-6" /> : <Bell className="w-5 h-5 sm:w-6 sm:h-6" />}
            </div>
            <p className="font-semibold text-xs sm:text-sm">{notification.message}</p>
          </div>
        </div>
      )}
      ...
      <nav className="bg-white dark:bg-gray-900 shadow-md sticky top-0 z-50 border-b border-gray-200 dark:border-gray-800">
        <div className="container mx-auto px-3 sm:px-4">
          <div className="flex items-center justify-between h-14 sm:h-16">
            ...
            {/* Desktop Search */}
            <div className="hidden md:block relative flex-1 max-w-md mx-4 lg:mx-8" ref={dropdownRef}>
              <input
                type="text"
                value={searchQuery}
            ...
            <div className="flex items-center gap-1 sm:gap-2 md:gap-3">
                  ...
                  <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl z-50 animate-fade-in">
                    {languages.map((lang) => (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-green-50 dark:hover:bg-gray-700 transition-colors ${currentLanguage === lang.code ? "bg-green-50 dark:bg-gray-700" : ""}`}
                      >
              ...
              {/* Wishlist */}
              <button onClick={() => handleNavClick('/wishlist')} className="relative p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors" disabled={wishlistLoading}>
              ...
              {/* Cart */}
              <button onClick={() => handleNavClick('/cart')} className="relative p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors" disabled={cartLoading}>
              ...
              {isAuthenticated ? (
                    ...
                    <div className="absolute right-0 mt-2 w-72 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl overflow-hidden z-50 animate-fade-in">
                      ...
                      <div className="bg-gradient-to-r from-green-600 to-emerald-700 p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-white font-semibold truncate">{user?.fullName || t('navbar.user')}</p>
                      ...
                      <div className="py-2">
                        {[
                          { path: '/profile', icon: User, color: 'blue', label: t('myProfile'), desc: t('navbar.viewProfile') },
                          { path: '/orders', icon: Package, color: 'purple', label: t('myOrders'), desc: t('navbar.trackOrders') },
                          { path: '/wishlist', icon: Heart, color: 'red', label: t('nav.wishlist'), desc: t('navbar.wishlistPreferences') },
                          { path: '/profile/settings', icon: Settings, color: 'green', label: t('nav.settings'), desc: t('navbar.accountPreferences') },
                        ].map((item) => (
                          <button key={item.path} onClick={() => handleNavClick(item.path)} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-green-50 dark:hover:bg-gray-700 transition-colors group">
                            <div className={`w-9 h-9 rounded-lg bg-${item.color}-100 dark:bg-${item.color}-900/30 flex items-center justify-center group-hover:scale-110 transition-transform`}>
                              <item.icon size={18} className={`text-${item.color}-600 dark:text-${item.color}-400`} />
                            </div>
                      ...
                      <div className="border-t border-gray-200 dark:border-gray-700 p-2">
                        <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors">
                ...
                <button onClick={() => handleNavClick('/login')} className="hidden sm:block px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-sm font-medium">
                  {t('login')}
                </button>
              )}
          ...
          {/* Mobile Search Bar */}
          {showMobileSearch && (
            <div className="md:hidden px-3 pb-3 pt-2 border-t border-gray-200 dark:border-gray-700" ref={dropdownRef}>
              <div className="relative">
                ...
                {isSearching && <div className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 border-2 border-green-600 border-t-transparent rounded-full animate-spin" />}
              </div>
              {showDropdown && searchResults.length > 0 && (
                <div className="mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg max-h-64 overflow-y-auto">
                  {searchResults.map((product) => (
                    ...
                    >
                      <img src={getImageUrl(product.images?.[0])} alt={product.name} className="w-10 h-10 object-cover rounded" onError={(e) => e.target.src = "/placeholder.png"} />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium truncate">{product.name}</p>
                        <p className="text-xs text-green-600">{t('currency')} {product.price}</p>
                      </div>
                    </button>
                  ))}
                </div>
            ...
            <div className="lg:hidden py-4 border-t border-gray-200 dark:border-gray-700">
              <div className="space-y-3">
                {['/', '/products', '/about', '/contact'].map((path) => (
                  <button key={path} onClick={() => handleNavClick(path)} className="block w-full text-left px-4 py-2 text-gray-700 dark:text-gray-300 hover:text-green-600 font-medium">
                    {path === '/' ? t('nav.home') : path === '/products' ? t('nav.products') : path === '/about' ? t('nav.about') : t('nav.contact')}
                  </button>
                ))}
                ...
                {!isAuthenticated && (
                  <button onClick={() => handleNavClick('/login')} className="mx-4 mt-3 block w-full py-2.5 bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium text-sm">
                    {t('login')}
                  </button>
                )}
              </div>
      ...
      <style jsx>{`
        ...
        .animate-slide-in-right { animation: slide-in-right 0.4s cubic-bezier(0.68, -0.55, 0.265, 1.55); }

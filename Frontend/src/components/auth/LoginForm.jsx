// ============================================
// LoginForm.jsx - WITH RIDER DASHBOARD ROUTING
    ...
    try {
      ...
      if (result?.success && result?.user) {
        ...
        if (userRole === 'admin' || userRole === 'superadmin' || user.isAdmin === true) {
          ...
          console.log('✅ LoginForm: Redirecting to rider dashboard')
          
          // Check if rider is approved
          if (user.riderProfile?.isApproved === false) {
            toast.error("Your rider account is pending approval. Please contact admin.")
            setLoading(false)
            return
          }
          ...
          toast.success(t('loginSuccess') || `Welcome back, ${user.firstname}!`)
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <p className="text-center text-sm text-gray-600 dark:text-gray-400">
        ...
        <a href="/register" className="text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 font-semibold">
          {t('register') || 'Sign up'}
        </a>
      </p>

      {/* ✅ NEW: Rider Registration Link */}
      <div className="relative">
